...
local function setAngularVelocity(x, y, z, pitchAV, rollAV, yawAV, onlyAngularVelocity, noCounterVelocity)
	...
	local rvdX = rvel.x - vehRvel.x
	local rvdY = rvel.y - vehRvel.y
	local rvdZ = rvel.z - vehRvel.z
	
	vel:set(x, y, z)
	vehVel:set(obj:getVelocityXYZ())
	cog:setCross(cog, vehRvel)
	vehVel:setAdd(cog)
	local vdX = vel.x - vehVel.x
	local vdY = vel.y - vehVel.y
	local vdZ = vel.z - vehVel.z
