...
local function processSlotsDestructive(part, sourceFilename)
	...
	if #part.slots > 0 and type(part.slots[1]) == 'table' and part.slots[1][1] ~= 'type' then
		...
		log('W', 'slotSystem',
			'Slot section of part ' ..
			tostring(part.partName) ..
			' in file ' ..
			tostring(sourceFilename) ..
			' misses the table header. Adding default: ["type", "default", "description"]. Please fix.')
	...
	for _, slot in ipairs(part.slots) do
		res[slot.name or slot.type] = { type = slot.type, description = slot.description, coreSlot = slot.coreSlot }
	end
	return res
end
	...
	for partName, part in pairs(fileContent) do
		...
		if addToCache then
			...
			if tableContains(partSlotMap[dir][part.slotType], partName) then
				...
				log('E', 'jbeam.loadJBeamFile',
					'Duplicate part found: ' .. tostring(partName) .. ' from file ' .. tostring(filename))
			else
				partFileMap[dir][partName] = filename
				partNameMap[dir][partName] = partDesc
	...
	for _, dir in ipairs(ioCtx.preloadedDirs) do
		...
		if jbeamFilename then
			if not jbeamCache[jbeamFilename] then
				...
				log('D', 'jbeam.getPart',
					"Loaded " .. tostring(partCount) .. " part(s) from file " .. tostring(jbeamFilename))
			end
	...
	for _, dir in ipairs(ioCtx.preloadedDirs) do
		...
		for partName, partDesc in pairs(partNameMap[dir]) do
			if res[partName] then
				log('E', "jbeam.getAvailableParts",
					"parts names are duplicate: " .. tostring(partName) .. ' in folders: ' .. dumps(ioCtx.preloadedDirs))
			end
			...
			for _, partName in ipairs(partList) do
				for _, partName2 in ipairs(res[slotName]) do
					if partName == partName2 then
						log('E', "jbeam.getAvailableSlotMap",
							"parts names are duplicate: " ..
							tostring(partName) .. ' in folders: ' .. dumps(ioCtx.preloadedDirs))
					end
				end
...
local function onFileChanged(filename, type)
	...
	if dir and (partFileMap[dir] or partSlotMap[dir] or partNameMap[dir]) then
		log('I', 'jbeamIO.onFileChanged',
			'cache reset for path: ' ..
			tostring(dir) .. ' due to file change: ' .. tostring(filename) .. ' (' .. tostring(type) .. ')')
		partFileMap[dir] = nil
		partSlotMap[dir] = nil
		partNameMap[dir] = nil
