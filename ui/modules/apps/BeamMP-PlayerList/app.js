...
app.controller("BeamMPPlayerListController", ['$scope', '$filter', 'Settings', function ($scope, $filter, Settings) {
	...
	$scope.init = function() {
		...
		bngApi.engineLua("guihooks.trigger('BeamMP_UpdateCustomButtons', UI.getCustomButtonNames())")
	...
	$scope.$on('BeamMP_UpdateCustomButtons', function(event, data) {
	...
	$scope.$on('BeamMP_SetNickname', function(event, data) {
		nickname = data
	})
